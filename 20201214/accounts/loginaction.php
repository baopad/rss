...
if ( !empty( $username ) && !empty( $password ) ) { //建立连接
    $conn = mysqli_connect( 'localhost', 'paotung', 'Pw000000', 'a' ); //准备数据库连接前置
    $sql_select = "SELECT username,password FROM b WHERE username = '$username' AND password = '$password'"; //用户名和密码搜索语句前置
    $ret = mysqli_query( $conn, $sql_select );//连接数据库并对比用户名和密码

    $row = mysqli_fetch_array( $ret ); //判断用户名或密码是否正确
    if ( $username == $row[ 'username' ] && $password == $row[ 'password' ] ) { //选中“记住我”
        if ( $remember == "on" ) { //创建cookie
